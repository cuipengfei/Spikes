...
public class NoDiscountTest {
    ...
    public void shouldConcatLineOutput() throws Exception {
        ...
        String output = noDiscount.output(new OrderLineItem(new Product("abc", "ItemXXX", 10d, "bottle", null), 10));

        assertThat(output, is("名称：abc，数量：10bottle，单价：10.00(元)，小计：100.00(元)"));
    }
}