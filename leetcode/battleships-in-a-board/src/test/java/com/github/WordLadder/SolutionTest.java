...
public class SolutionTest {
    @Test
    public void shouldTellIfIsOneLetterDiff() throws Exception {
        boolean isOneLetterDiff = Solution.isOneLetterDiff("hot", "hit");
        assertTrue(isOneLetterDiff);

        isOneLetterDiff = Solution.isOneLetterDiff("hot", "hao");
        assertFalse(isOneLetterDiff);
    }
    ...
    public void exampleInput() throws Exception {
        ...
        assertThat(ladders.get(0), CoreMatchers.hasItems("hit", "hot", "lot", "log", "cog"));
        assertThat(ladders.get(1), CoreMatchers.hasItems("hit", "hot", "dot", "dog", "cog"));
    }

    @Test
    public void anotherInput() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("git", "php",
                new HashSet<>(Arrays.asList("gip", "pip", "pit")));

        assertThat(ladders.get(0), CoreMatchers.hasItems("git", "pit", "pip", "php"));
        assertThat(ladders.get(1), CoreMatchers.hasItems("git", "gip", "pip", "php"));
    }

    @Test
    public void shouldAvoidBackLooping() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("leet", "code",
                new HashSet<>(Arrays.asList("lest", "leet", "lose", "code", "lode", "robe", "lost")));

        assertThat(ladders.get(0), CoreMatchers.hasItems("leet", "lest", "lost", "lose", "lode", "code"));
    }

    @Test
    public void oneLetterWords() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("a", "c",
                new HashSet<>(Arrays.asList("a", "b", "c")));

        assertThat(ladders.get(0), CoreMatchers.hasItems("a", "c"));
    }

    @Test
    public void onlyReturnShortest() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("kiss", "tusk",
                new HashSet<>(Arrays.asList("kiss", "diss", "disk", "dusk", "tusk", "miss", "muss", "musk")));

        assertThat(ladders.get(0), CoreMatchers.hasItems("kiss", "diss", "disk", "dusk", "tusk"));
        assertThat(ladders.get(1), CoreMatchers.hasItems("kiss", "miss", "muss", "musk", "tusk"));
    }

    @Test
    public void onlyReturnShortest2() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("cat", "fin",
                new HashSet<>(Arrays.asList("cat", "fat", "fit", "fin", "fan", "can", "mat", "mit")));

        assertThat(ladders.get(0), CoreMatchers.hasItems("cat", "can", "fan", "fin"));
    }

    @Test
    public void shouldHandleNoAnswerInput() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("hot", "dog",
                new HashSet<>(Arrays.asList("hot", "dog")));

        assertThat(ladders.size(), is(0));
    }

    @Test
    public void shouldHandleBigInput() throws Exception {
        Solution solution = new Solution();

        List<List<String>> ladders = solution.findLadders("sand", "acne",
